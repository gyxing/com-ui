<template>
    <span class="sim-loading" :class="'sim-loading-'+type" :style="styleObj">
...
<script>
    export default {
        name: "Loading",
        props: {
            type: { type: String, default: () => 'inline' },  // inline、box
            text: { type: String },
            color: { type: String },
            size: { type: String }
        },
        computed: {
            styleObj() {
                ...
                if(this.size) {
                    let p = this.size.indexOf('%') !== -1 ? '' : 'px';
                    s.width = this.size + p;
                    s.height = this.size + p;
                }
                return s;
            }
        }
